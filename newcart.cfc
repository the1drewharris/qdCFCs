<cfcomponent hint="cart - functions">
	...
	<cffunction name="getallNamecarts" access="public" returntype="query" output="false" hint="I get all the active carts">
		...
		<cfquery name="allcarts" datasource="#cartdsn#">
		 	...
		 	WHERE CARTOWNER.NAMEID=NAME.NAMEID
	...
	<cffunction name="getCartByCreatedbyid" returntype="any" access="public" output="false" hint="I get the cartid for the createdbyid passed to me"> 
		...
		<cfargument name="taxrate" type="String" required="false" default="0" hint="The taxrate for the transaction">
		<cfargument name="frieght" type="string" required="false" default="0" hint="The cost of frieght for this transaction">
		...
		<cfif getMyCart.recordcount GT 0>
			<cfinvoke method="getCart" cartdsn="#cartdsn#" taxrate="#arguments.taxrate#" pricename="#arguments.pricename#" cartid="#getMyCart.cartid#" returnvariable="myxml">
			<cfreturn myxml>
		</cfif>
		<cfreturn "Error">
	</cffunction>
	...
	<cffunction name="getCart" returntype="any" access="public" output="false" hint="I get all of the cart data for the cartid passed to me">
		...
		<cfargument name="taxrate" type="String" required="false" default="0" hint="The taxrate for the transaction">
		<cfargument name="freight" type="string" required="false" default="0" hint="The cost of frieght for this transaction">
		<cfargument name="pricename" type="String" false default="Retail" hint="Price name to be applied">
		<cfset var viewcart=0>
		<cfset var carttotal = 0>
		<cfset var subtotal = 0>
		<cfset var subtotalweight = 0>
		<cfset var totaldiscount = 0>
		<cfset var couponcode = 0>
		<cfif arguments.cartdsn eq "merrittsbakery.com">
			<cfset arguments.pricename="Standard">
		</cfif>
		<cfif arguments.cartdsn eq "vcientertianment.com">
			<cfset arguments.pricename="Web">
		</cfif>
			...
			<mycart>
				...
				<cfoutput query="viewcart">
					<item 
						...
						id="#id#" 
						itemnumber="#clientproductid#"
						quantity="#quantity#"
						itemtype="#idfor#"
						price="#unitprice#"
						weight="#weight#"
						<cfif isDefined ('cookie.cart')>
							<cfif viewcart.recordcount gt 0>
							<!--- discount="#XmlParse(Cookie.cart).mycart.item[viewcart.currentrow].XmlAttributes.discount#" --->
							discount="0"
							<cfelse >
							discount="0"
							</cfif>
					...
					<cfset subtotal = subtotal + unitprice*quantity>
					<cfif isnumeric(weight)>
						<cfset thisweight=weight>
					<cfelse>
						<cfset thisweight = 0>
					</cfif>
					<cfset subtotalweight = subtotalweight + thisweight>
				</cfoutput>
				<cfoutput>
					<cfif isDefined('cookie.cart')>
						<cfif cookie.cart neq "" and cookie.cart neq 0>
						<cfset couponcode="#XmlParse(Cookie.cart).mycart.coupouncode.XmlText#">
						</cfif>
					<cfelse>
						<cfset couponcode=0>
					</cfif>
					...
					<cfif isDefined('cookie.cart')>
						<cfif cookie.cart neq "" and cookie.cart neq 0>
						<cfset totaldiscount="#XmlParse(Cookie.cart).mycart.totaldiscount.XmlText#">
						</cfif>
					<cfelse>
						<cfset totaldiscount=0>
					</cfif>
					
					<cfset discountedTotal=subtotal - totaldiscount>
					<cfset taxTotal=discountedTotal * arguments.taxrate>
					<cfset afterTaxTotal=discountedTotal + taxTotal>
					<cfset carttotal = afterTaxTotal + arguments.freight>
					<totalweight>#subtotalweight#</totalweight>
					<noofitems>#viewcart.recordcount#</noofitems>
					<grosstotal>#subtotal#</grosstotal>
					<coupouncode>#couponcode#</coupouncode>
					<totaldiscount>#totaldiscount#</totaldiscount>
					<taxtotal>#taxTotal#</taxtotal>
					<freightTotal>#arguments.freight#</freightTotal>
