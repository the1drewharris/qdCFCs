<cfcomponent hint="New Forum CFC">
	...
	<cffunction name="notifySubscribers" access="public" returntype="void" hint="I email subscribers">
		...
		<cfset var emaillist="">
		<cfset var sender="">
		<cfif isDefined('application.emailadmin')>
			<cfset sender=application.emailadmin>
		<cfelse>
			<cfset sender="support@#arguments.ds#">
		</cfif>
		...
		<cfif len(namelist)>
			...
			<cfloop list="#emaillist#" index="emailaddress">
				...
				<cfmail to="#emailaddress#" from="#sender#" subject="New post made on topic you subscribed to" type="html">
					Dear Forum user, <br />
	...
	<cffunction name="SearchForum" access="public" returntype="query" hint="I search">
		...
		<cfset arguments.keywords=rereplace(arguments.keywords,"'","","All")>
		<cfset arguments.keywords=rereplace(arguments.keywords,"\+",",","All")>
...
<!--- 		<cfset thispost=REReplaceNocase(thispost,"\[quote=&quot;(.*?)&quot;:(.*?)\](.*?)\[/quote:(.*?)\](.*?)","<div style='background-color:##ccc;border-style:solid;border-width:1px;padding:10px'>\1 Wrote: <br /> \3 </div> \5","All")>
 --->	
	...
	<cffunction name="replypost" access="public" returntype="string" output="true" hint="I add post">
		...
		<cfargument name="subscribe" type="string" required="false" default="0" hint="0:Do not subscribe, 1: Subscribe">
		<cfargument name="notifySubscribers" type="string" required="false" default="1" hint="0: Do not notify">
		<cfset var subscribetotopic=0>
		<cfset var getname=0>
		<cfset var add=0>
		<cfset var get=0>
		...
		<cfif controlcode EQ 0>
			<cfif arguments.notifySubscribers EQ 1>
				<cfset args=StructNew()>
				<cfset args.ds=arguments.ds>
	...
	<cffunction name="approveposts" access="public" returntype="string" output="false" hint="I approve forum post">
		...
		<cfset var timedate=mytime.createtimedate()>
		<cfset args.ds=arguments.ds>
		<cfloop list="#postList#" index="postid">
			...
			<cfquery name="approve" datasource="#arguments.ds#">
				UPDATE POSTS SET 
					POSTAPPROVED=<cfqueryparam value="1">,
					POSTTIME=<cfqueryparam value="#timedate#">
			...
			<cfquery name="getinfo" datasource="#arguments.ds#">
				...
				WHERE NAMEID=(SELECT POSTERID FROM POSTS WHERE POSTID=<cfqueryparam value="#postid#">)
			</cfquery>
	...
	<cffunction name="sendprivatemsgs" access="public" returntype="string" output="true" hint="I send private message">
		...
		<cfquery name="send" datasource="#arguments.ds#">
			...
			VALUES
			(
				<cfqueryparam value="#Val(arguments.msgsid)#">,
				<cfqueryparam value="#Val(arguments.msgsfrom)#">,
				<cfqueryparam value="#Val(arguments.msgsto)#">,
				-3
			)
		</cfquery>
		<cfquery name="send" datasource="#arguments.ds#">
