<cfcomponent hint="New Forum CFC">
	...
	<cffunction name="createForumTables" access="public" returntype="void" hint="I add the new blog tables to database">
		<cfargument name="ds" type="string" required="true" hint="I am the dsn you want to create tables for">
		<cfset var createqdcmsFORUM=0>
		<cfquery name="createqdcmsFORUM" datasource="#arguments.ds#">
	...
	<cffunction name="notifySubscribers" access="public" returntype="void" hint="I email subscribers">
		...
		<cfif len(namelist)>
			...
			<cfloop list="#emaillist#" index="emailaddress">
				...
				<cfmail to="#emailaddress#" from="#application.emailadmin#" subject="New post made on topic you subsribed to" type="html">
					Dear Forum user, <br />
	...
	<cffunction name="SearchResultCount" access="public" returntype="string" hint="I search">
		<cfargument name="keywords" required="true" type="string" hint="words to look for">
		...
		<cfset var get=0>
		<cfset lastkeyword=listlast(arguments.keywords,",+ ")>		
		<cfset upperlimit=arguments.lowerlimit + 10>
		...
		<cfquery name="get" datasource="#arguments.ds#">
			SELECT
			count(POSTID) AS NOOFRECORDS
			FROM POSTS, NAME
			WHERE POSTS.POSTAPPROVED=1
			AND POSTS.POSTDELETED <>1
 ...
 --->	
	...
	<cffunction name="deleteforum" access="public" returntype="string" output="false" hint="I edit forum">
		...
		<cfquery name="delete" datasource="#arguments.ds#">
			UPDATE FORUMS SET STATUS=0 WHERE FORUMID IN (#arguments.forumlist#)
		</cfquery>
		<cfreturn>
	</cffunction>
	...
	<cffunction name="Undeleteforum" access="public" returntype="string" output="false" hint="I edit forum">
		...
		<cfquery name="undelete" datasource="#arguments.ds#">
			UPDATE FORUMS SET STATUS=1 WHERE FORUMID IN (#arguments.forumlist#)
		</cfquery>
		<cfreturn>
	</cffunction>
	...
	<cffunction name="getForums" access="public" returntype="query" output="false" hint="I return forum;FORUMID,NAME,DESCRIPTION,CREATEDON,STATUS,LASTUPDATED,NOOFTOPICS,NOOFPOSTS,LASTPOSTID,LASTPOSTERID,LASTPOSTERNAME,LASTPOSTDATE,ROW">
		<cfargument name="ds" required="true" type="string" hint="I am the the datasource for the forum">
		...
		<cfargument name="status" required="false" type="string" default="1" hint="status of the ">
		<cfset var get=0>
		<cfset upperlimit=arguments.lowerlimit + 25>
		<cfquery name="get" datasource="#arguments.ds#">
			SELECT * FROM
			(
			SELECT 
	...
	<cffunction name="addtopic" access="public" returntype="string" output="false" hint="I add topic">
		...
		<cfquery name="get" datasource="#arguments.ds#">
			SELECT FIRSTNAME FROM NAME WHERE NAMEID=<cfqueryparam value="#arguments.topicposter#" cfsqltype="cf_sql_varchar">
		</cfquery>
		<cfquery name="add" datasource="#arguments.ds#">
			INSERT INTO TOPICS
			(
			...
			(
				...
				<cfqueryparam value="#get.firstname#" cfsqltype="cf_sql_varchar">,
		...
		<cfquery name="editforum" datasource="#arguments.ds#">
			UPDATE FORUMS
			SET NOOFTOPICS=(SELECT COUNT(*) AS NOOFTOPICS FROM TOPICS WHERE FORUMID = <cfqueryparam value="#arguments.forumid#"> AND TOPICDELETED=0) 
			WHERE FORUMID=<cfqueryparam value="#arguments.forumid#">
		</cfquery>
		<cfreturn add.TOPICID>
	</cffunction>

	<cffunction name="getTopics" access="public" returntype="query" output="false" hint="I return forum topics">
		<cfargument name="ds" required="true" type="string" hint="I am the the datasource for the forum">
		...
		<cfargument name="approved" required="false" type="string" default="1" hint="I get all topics. If approved is 0, I get only unapproved topics">
		<cfargument name="orderby" required="false" type="string" default="LASTPOSTDATE" hint="Order by for forum">
		...
		<cfargument name="deleted" required="false" type="string" default="0" hint="1 if you are looking for deleted topics">
		<cfset var get=0>
		<cfset upperlimit=arguments.lowerlimit + 25>
		...
		<cfquery name="get" datasource="#arguments.ds#">
			...
			<cfif recordlimit NEQ 0>top #recordlimit#</cfif>
				...
				ROW_NUMBER() OVER (ORDER BY #orderby# #ascdesc#) AS ROW
			FROM TOPICS
			WHERE TOPICAPPROVED=<cfqueryparam value="#arguments.approved#" cfsqltype="cf_sql_varchar">
			AND TOPICDELETED=<cfqueryparam value="#arguments.deleted#" cfsqltype="cf_sql_varchar">
			<cfif arguments.forumid NEQ "0">
			AND FORUMID=<cfqueryparam value="#arguments.forumid#" cfsqltype="cf_sql_varchar">
			</cfif>
			<cfif arguments.topicid NEQ "0">
		...
		<cfquery name="get" datasource="#arguments.ds#">
			SELECT * FROM (
				SELECT
					...
					ROW_NUMBER() OVER (ORDER BY #orderby# #ascdesc#) AS ROW
				FROM POSTS, NAME
				WHERE POSTS.POSTAPPROVED=<cfqueryparam value="#arguments.approved#" cfsqltype="cf_sql_varchar">
				AND POSTS.POSTERID=NAME.NAMEID
				...
				OR POSTS.POSTTEXT LIKE <cfqueryparam value="%#arguments.keyword#%" cfsqltype="cf_sql_varchar">)
				</cfif>
				) ALLPOSTS WHERE ROW > <cfqueryparam value="#arguments.lowerlimit#"> AND ROW <= <cfqueryparam value="#upperlimit#">
		</cfquery>
		<cfreturn get>
	...
	<cffunction name="approveposts" access="public" returntype="string" output="false" hint="I approve forum post">
		...
		<cfloop list="#postList#" index="postid">
			<cfquery name="get" datasource="#arguments.ds#">
				SELECT TOPICID FROM POSTS WHERE POSTID=<cfqueryparam value="#postid#" cfsqltype="cf_sql_varchar">
			</cfquery>
	...
	<cffunction name="deleteposts" access="public" returntype="string" output="false" hint="I delete forum topic.">
		...
		<cfloop list="#postList#" index="postid">
			...
			<!--- decrease the noofreplies if it is not the first post --->
			<cfquery name="check" datasource="#arguments.ds#">
				SELECT TOP 1 POSTID FROM POSTS WHERE TOPICID=<cfqueryparam value="#get.TOPICID#" cfsqltype="cf_sql_varchar"> ORDER BY POSTID
			</cfquery>
			<cfif check.POSTID NEQ postid>
				<cfquery name="update" datasource="#arguments.ds#">
					UPDATE TOPICS SET NOOFREPLIES=((SELECT COUNT(*) AS NOOFPOSTS FROM POSTS WHERE TOPICID = <cfqueryparam value="#get.topicid#" cfsqltype="cf_sql_varchar"> AND POSTDELETED=0)-1)
					WHERE TOPICID=<cfqueryparam value="#get.topicid#" cfsqltype="cf_sql_varchar">
				</cfquery>
			</cfif>
			<!--- get the count of posts for the topic --->
			<cfquery name="countpost" datasource="#arguments.ds#">
				SELECT COUNT(POSTID) AS NOOFPOSTS FROM POSTS WHERE TOPICID=<cfqueryparam value="#get.topicid#" cfsqltype="cf_sql_varchar"> AND POSTDELETED<>1
			</cfquery>
			
			<!--- if noofposts is 0, delete the topic --->
			<cfif countpost.noofposts EQ 0>
				<cfquery name="delete" datasource="#arguments.ds#">
					UPDATE TOPICS SET TOPICDELETED=1 WHERE TOPICID=<cfqueryparam value="#get.topicid#" cfsqltype="cf_sql_varchar">
				</cfquery>
			</cfif>
	...
	<cffunction name="Undeleteposts" access="public" returntype="string" output="false" hint="I delete forum topic.">
		...
		<cfloop list="#postList#" index="postid">
			...
			<!--- increase the noofreplies if it is not the first post --->
			<cfquery name="check" datasource="#arguments.ds#">
				SELECT NOOFREPLIES FROM TOPICS WHERE TOPICID=<cfqueryparam value="#get.TOPICID#" cfsqltype="cf_sql_varchar">
			</cfquery>
			
			<cfif check.NOOFREPLIES NEQ 0>
				<cfquery name="update" datasource="#arguments.ds#">
					UPDATE TOPICS SET NOOFREPLIES=((SELECT NOOFREPLIES FROM TOPICS WHERE TOPICID = <cfqueryparam value="#get.topicid#">) + 1)
				</cfquery>
			<cfelse>
				<cfquery name="update" datasource="#arguments.ds#">
					UPDATE TOPICS SET TOPICDELETED=0 WHERE TOPICID=<cfqueryparam value="#get.topicid#" cfsqltype="cf_sql_varchar">
				</cfquery>
			</cfif>
	...
	<cffunction name="undeletetopics" access="public" returntype="string" output="false" hint="I delete a topic">
		...
		<cfloop list="#arguments.topiclist#" index="topicid">
			...
			<cfquery name="update" datasource="#arguments.ds#">
				UPDATE FORUMS SET 
					NOOFPOSTS=((SELECT NOOFPOSTS FROM FORUMS WHERE FORUMID = <cfqueryparam value="#get.forumid#">) + #countposts.noofposts#),
					NOOFTOPICS=((SELECT NOOFTOPICS FROM FORUMS WHERE FORUMID = <cfqueryparam value="#get.forumid#">) + 1)
				WHERE FORUMID=<cfqueryparam value="#get.forumid#" cfsqltype="cf_sql_varchar">
			</cfquery>
		</cfloop>
	</cffunction>
	...
	<cffunction name="getchildren" access="public" returntype="query" hint="I get all subforums of the forum;FORUMID, NAME, DESCRIPTION">
		...
		<cfquery name="get" datasource="#arguments.ds#">
			SELECT FORUMID FROM FORUMS_RELATION
		...
		<cfquery name="getchildren" datasource="#arguments.ds#">
			SELECT FORUMID, NAME, DESCRIPTION 
			FROM FORUMS
	...
	<cffunction name="getLastPost" access="public" returntype="query" hint="I return last post;POSTID,POSTERID,USERNAME">
		...
		<cfquery name="get" datasource="#arguments.ds#">
			...
			AND POSTS.POSTDELETED=0
			<cfif arguments.forumid NEQ 0>
			AND POSTS.FORUMID=<cfqueryparam value="#arguments.forumid#" cfsqltype="cf_sql_varchar">
			</cfif>
			<cfif arguments.topicid NEQ 0>
			AND POSTS.TOPICID=<cfqueryparam value="#arguments.topicid#" cfsqltype="cf_sql_varchar">
			</cfif>
