...
<cfcomponent hint="video library functions">
	...
	<cffunction name="addVideo" access="public" returntype="string" hint="I add video to the video library">
		...
		<cfargument name="mediatypeid" required="false" type="string"  default="1" hint="Caption of the video">
	...
	<cffunction name="getVideos" access="public" returntype="query" hint="I get the videos from the database. Output fields: VID,TITLE, STATUS,CAPTION,LINK,LINKTEXT,KEYWORDS,VIDEOPATH,IMAGEPATH,SUBTITLE,MEDIATYPEID,SUMMARY,DESCRIPTION,SAMPLEID,SAMPLEPATH,CREATEDON,UPDATEDON,SORTORDER,LASTVIEWED">
		<cfargument name="videodsn" required="true" type="string" hint="Datasource">
		<cfargument name="videocategoryid" required="false" type="string" hint="videocategoryid of the videos">
		...
		<cfif not isDefined('arguments.videocategoryid')>
			<cfif category NEQ "All">
				<cfinvoke method="getcategoryid" 
						  videodsn="#videodsn#"
						  category="#category#"
						  returnvariable="videocategoryid">
			</cfif>
		<cfelse>
	...
	<cffunction name="getCategory" access="public" returntype="query" hint="I get number of videos that belong to the category. Output: category, videocount, imagepath">
		...
		<cfargument name="excludelist" type="any" required="false" hint="list of categoryid to exclude">
		<cfset var categoryinfo=0>
		<cfquery name="categoryinfo" datasource="#videodsn#">
			SELECT 
				VIDEOCATEGORY.CATEGORY,
				VIDEOCATEGORY.VIDEOCATEGORYID,
				ROW_NUMBER() OVER (ORDER BY VIDEOCATEGORY.SORTORDER) AS ROW,
				(SELECT COUNT(VID) 
				FROM VIDEO_TO_CATEGORY 
				WHERE VIDEOCATEGORY.VIDEOCATEGORYID=VIDEO_TO_CATEGORY.VIDEOCATEGORYID
				AND VID IN(SELECT VID FROM VIDEOLIB 
				WHERE MEDIATYPEID=<cfqueryparam value="#arguments.mediatypeid#" cfsqltype="cf_sql_varchar">
				AND STATUS='Public')
				) AS VIDEOCOUNT,
				(SELECT IMAGEPATH 
				FROM VIDEOLIB 
				...
				IN(SELECT TOP 1 VID 
				  FROM VIDEO_TO_CATEGORY 
				  WHERE VIDEOCATEGORYID=VIDEOCATEGORY.VIDEOCATEGORYID 
				  ORDER BY NEWID())) AS IMAGEPATH
			FROM VIDEOCATEGORY
			WHERE 1=1
			<cfif criteria neq 0>
			AND VIDEOCATEGORY.CATEGORY LIKE <cfqueryparam value="%#criteria#%" cfsqltype="cf_sql_varchar">
			</cfif>
			<cfif isDefined('arguments.excludelist')>
			AND VIDEOCATEGORY.VIDEOCATEGORYID NOT IN(#arguments.excludelist#)
			</cfif>
			ORDER BY VIDEOCATEGORY.SORTORDER
		</cfquery>
	...
	<cffunction name="getsamples" access="public" returntype="Query" hint="I get the samples of the media">
		...
		<cfargument name="vid" type="string" required="true" hint="vid of a video">
		<cfset var samples=0>
		<cfquery name="samples" datasource="#videodsn#">
			SELECT SAMPLEID, 
	...
	<cffunction name="getcategoryname" output="false" access="public" returntype="query" hint="I get category from videocategoryid. outputs: videocategoryid, category, noofvideos, sortorder and imagepath">
		...
		<cfquery name="getcategoryname" datasource="#arguments.videodsn#">
			SELECT 
				...
				WHERE VIDEOCATEGORYID=<cfqueryparam value="#arguments.videocategoryid#" cfsqltype="cf_sql_varchar">
				AND VID IN(SELECT VID FROM VIDEOLIB WHERE MEDIATYPEID=<cfqueryparam value="#arguments.mediatypeid#" cfsqltype="cf_sql_varchar">)) AS NOOFVIDEOS,
	...
	<cffunction name="isSubscriableProduct" output="false" access="public" returntype="Numeric" hint="I find if a video is a subscriable product">
		...
		<cfquery name="check" datasource="#videodsn#">
			...
			WHERE SUBSCRIABLEID=<cfqueryparam value="#vid#" cfsqltype="cf_sql_varchar">
		</cfquery>	
		...
		<cfsavecontent variable="xml_map"><?xml version="1.0" encoding="UTF-8" standalone="yes"?>
			<VideoLibrary>
				<cfoutput query="qryvideos">
					...
					<cfinvoke method="getVideoCategory" returnvariable="categories" videodsn="#videodsn#" vid="#vid#">
	...
	<!--- function to build xml database for SSP --->
	<cffunction name="SSPXmlBuild" output="false" returntype="xml" access="public" hint="I build the xml for the videodsn passed to me.">
		...
		<cfsavecontent variable="ssp_xml"><?xml version="1.0" encoding="UTF-8" standalone="yes"?>			
			<gallery>
				<cfoutput><album id="#arguments.galleryid#" lgPath="http://#arguments.videodsn#/videos/" tnPath="http://#arguments.videodsn#/images/small/" videocount="#qryvideos.recordcount#" ></cfoutput>
				<cfoutput query="qryvideos">
