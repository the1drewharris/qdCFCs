...
<cfinclude template="StripAllBut.cfm">
	...
	<cffunction name="getImageStatus" access="public" returntype="string" hint="I get status of an image">
		...
		<cfquery name="get" datasource="#client.siteurl#">
			...
			WHERE IMAGEID=<cfqueryparam value="#form.imageid#">
		</cfquery>
		<cfreturn get.STATUS>
	</cffunction>
	...
	<cffunction name="uploadImage" output="false" returntype="struct" access="public" hint="I upload the image sent to me.  I return a structure called myImage.">
		...
		<cfargument name="FileName" optional="False" hint="The filefield used for cffile upload to work properly"/>	
		<cfargument name="allowedExtenstions" required="false" default="jpg,png,gif,jpeg">
		<cfargument name="maxFileSize" required="false" default="0" hint="Maximum File size allowed for uploaded file (in Mbs).">
		<cfset var newImageName=0>
		<cfset var name=0>
		<cfset var myImage=StructNew()>
		<cfset myImage['error'] = 0>
		<cfset myImage['name'] = 0>
		<cfinvoke component="imagegallery" method="dirCheckCreate" myDir="#arguments.path#">
		<CFFILE action="upload" FILEFIELD="#arguments.FileName#" destination="#arguments.path#" nameconflict="rename" mode="775" result="myNewImage">
		<cfif myNewImage.filewassaved eq "NO">
			<cfset myImage['error'] = "Your file was not saved to the server">		
		<cfelse>
			<cfset name = "#myNewImage.serverfile#">
			<cfset newname=listfirst(#name#, ".")>
			<cfset ext=lcase(listlast(#name#, "."))>
				<!--- I am used to strip image names down to fit. --->
				<cfset lowername = "#lcase(newname)#">
				<cffile action="rename" source="#arguments.path#/#myNewImage.serverfile#" destination="#arguments.path#/a_#lowername#.#ext#">
				<cffile action="rename" source="#arguments.path#/a_#lowername#.#ext#" destination="#arguments.path#/#lowername#.#ext#">
				<cfset newImagename = "#StripAllBut(lowername, "1234567890abcdefghijklmnopqrstuvwxyz_ ")#">
				<cfif FindNoCase(" ", "#newimagename#")>
					<cfset newImagename = "#ReplaceNoCase(newimagename, " ", "_", "ALL")#">
				</cfif>
				<cfif len(#newImagename#) gt 31>
					<cfif mid(#newImagename#, 31, 1) eq "_">
						<cfset newImagename = "#left(newimagename, 31)#">
					<cfelse>
						<cfset newImagename = "#left(newimagename, 32)#">
					</cfif> 
				</cfif>
				<cfif isNumeric(left(#newImagename#,1))>
					<cfset newImagename="a#newimagename#.#ext#">
				<cfelse>
					<cfset newImagename="#newimagename#.#ext#">
				</cfif>
				<!--- rename the image if needed --->
				<cfif FileExists("#arguments.path#/#lowername#.#ext#") and myNewImage.serverfile neq newimagename>
					<cffile action="rename" source="#arguments.path#/#lowername#.#ext#" destination="#arguments.path#/#NEWIMAGENAME#">
				</cfif>
		</cfif>
		
		<cfif myNewImage.filewassaved EQ "yes">
			<!---Check the file extension against our list of allowed extensions --->
			<cfif NOT listContainsNoCase(arguments.allowedExtenstions, ext) or NOT IsImageFile("#arguments.path#/#newimagename#")>
				<cffile action="delete" file="#arguments.path#/#newimagename#">
				<cfset myImage['error'] = "You attempted to upload file with extension #ext#. The file format you have attempted to upload is not allowed and has not been uploaded">
			</cfif>
			<!--- Check the file size to make sure its not larger than max --->
			<cfif myNewImage.fileSize GT (maxFileSize * 1024 * 1024) AND maxFileSize NEQ 0>
   			<!--- <cfset myImage['error'] = "The file must be smaller than #maxFileSize#Mb"> --->
			 <!--- new rule, resize this image, binod@quantumdelta.com, 20090701 --->
			 <cfset percentage=(maxFileSize*1024*1024)/myNewImage.fileSize>
			 <cfset percentage="#Int(percentage*100)#%">
			 	<cfimage action="resize" source="#arguments.path#/#newImagename#" width="#percentage#" height="#percentage#" destination="#arguments.path#/#newimagename#" overwrite="yes"> 
			</cfif>
		</cfif>
	...
	<cffunction name="createImageThumb" output="no" returntype="string" hint="Creates a thumbnail of a master image and return the full path and name of that image">
		...
		<cfset sourceImg = "#arguments.masterpath##arguments.masterimage#">
       	...
       	<cfinvoke method="dirCheckCreate" myDir="#arguments.masterpath#">
		<cfinvoke method="dirCheckCreate" myDir="#arguments.thumbdir#">
	...
	<cffunction name="getGalleryImages" output="false" returntype="query" access="public" hint="I get the images that match the criteria passed to me, I return a recordset, (IMAGEID, CAPTION, IMAGEPATH, ALT, ALIGN, HSPACE, VSPACE, HEIGHT, WIDTH, TITLE, IMAGENAME, IMAGECLASS, SORTORDER, FLASHGALLERY, LINK, CREATEDBYID, VERSIONID, VERSIONDESCRIPTION, IMAGECATEGORY, IMAGECATEGORYID, PARENTIMAGECATEGORYID, DESCRIPTION, STATUS)">
		...
		OR IMAGE.IMAGEPATH LIKE '%.GIF')
		AND IMAGECATEGORY.IMAGECATEGORYID = IMAGE2IMAGECATEGORY.IMAGECATEGORYID	
		<cfif isdefined('excludeList')>AND IMAGEID NOT IN (#excludeList#)</cfif>
		<cfif isDefined('galleryid')>AND IMAGECATEGORY.IMAGECATEGORYID = <cfqueryparam value="#galleryid#"></cfif>
		<cfif isdefined('status')>AND IMAGE.STATUS = <cfqueryparam value="#status#"></cfif>
		<cfif isdefined('showinflashgallery')>AND IMAGE.FLASHGALLERY = <cfqueryparam value="#showinflashgallery#"></cfif>
		<cfif isdefined('sortimages')>
		order by #sortimages#
		<cfelse>
	...
	<cffunction name="getGalleryInfo" output="false" returntype="query" access="public" hint="I get the information for the galleryid passed to me, I return a query (IMAGECATEGORYID, SORTORDER, IMAGECATEGORY, DESCRIPTION, PARENTIMAGECATEGORYID, STATUS, VERSIONID)">
		<cfargument  name="imagedsn" type="string" required="true" hint="The datasource for the images">
		<cfargument name="galleryid" type="Numeric" required="true" hint="ID of the gallery you want images from">
	...
	<cffunction name="updateImagePath" output="false" returntype="boolean" access="public" hint="I update the image path in the database">
		...
		<cfargument name="newimagepath" type="string" required="true" hint="The new image path of the image to be updated">
			<cfinvoke component="imagegallery" imagedsn="#imagedsn#" method="getImage" imagepath="#arguments.oldimagepath#" returnVariable="image">
			<cfif image.recordCount EQ 1>
				<cfquery name="updateImagePath" datasource="#imagedsn#">
					UPDATE IMAGE
	...
	<cfset tempDir = ListAppend('#baseImagePath#', 'temp', '/')>
		...
		<cfif FileExists("#baseImagePath#/#arguments.oldImageName#")>
			...
			<cfif arguments.copy eq 0>
				<cffile action="rename" source="#baseImagePath#/#oldImageName#" destination="#baseImagePath#/#myNewImage#" nameconflict="overwrite" mode="775" >
				<cfloop list="#thumbdirlist#" index="currFolder">
					<cfif FileExists("#baseImagePath#/#currFolder#/#oldImageName#")>
					<cffile action="rename" source="#baseImagePath#/#currFolder#/#oldImageName#" destination="#baseDestinationPath#/#currFolder#/#myNewImage#" nameconflict="overwrite" mode="775" >
					</cfif>
				</cfloop>
