<cfcomponent hint="I have all functions for survey">
	...
	<cffunction name="editRelatedQuestion" access="public" hint="I edit Related Question">
		<cfargument name="surveydsn" required="true" type="string" hint="database name">
		<cfargument name="qrelationid" required="true" type="string" hint="id of the relation">
		<cfargument name="answerchoiceid" required="true" type="string" hint="id of the answerchoice">
		<cfargument name="related_questionid" required="true" type="string" hint="id of the related question">
		<cfset var get = 0>
		<cfquery name="get" datasource="#arguments.surveydsn#">
			UPDATE QRELATION SET
				ANSWERCHOICEID=<cfqueryparam value="#arguments.answerchoiceid#" cfsqltype="cf_sql_varchar">,
				RELATED_QUESTIONID=<cfqueryparam value="#arguments.related_questionid#" cfsqltype="cf_sql_varchar">
			WHERE QRELATIONID=<cfqueryparam value="#arguments.qrelationid#" cfsqltype="cf_sql_varchar">		
		</cfquery>
	</cffunction>
	...
	<cffunction name="getSurveys" access="public" returntype="query" hint="I show all surveys created by name id. If nameid is not passed, I show all surveys. Return fields: SURVEYID,SURVEYNAME,NUMBEROFTIMESFOR1IP,NUMBEROFTIMESFORNAMEID,STARTDATE,ENDDATE">
		...
		<cfquery name="getsurveys" datasource="#surveydsn#">
			SELECT 
				...
				ALLOWENDDATE,
				ACTIVE,
				(SELECT COUNT(QUESTIONID) FROM QUESTIONS Q WHERE Q.SURVEYID=S.SURVEYID) AS NOOFQUESTIONS
			FROM SURVEY S
	...
	<cffunction name="editSurvey" access="public" returntype="void" hint="I update the survey">
		...
		<cfargument name="allowenddate" required="false" default="0" type="String" hint="to set and end date or not">
		<cfargument name="active" required="false" default="0" type="String" hint="Status of the survey, whether active or not">
		<cfquery name="editsurvey" datasource="#surveydsn#">
			UPDATE SURVEY
			SET 
			SURVEYNAME=<cfqueryparam value="#arguments.surveyname#" cfsqltype="cf_sql_varchar">
			...
			<cfif isDefined('arguments.numberoftimesfor1ip')>
			,NUMBEROFTIMESFOR1IP=<cfqueryparam value="#arguments.numberoftimesfor1ip#" cfsqltype="cf_sql_integer">
			</cfif>
			<cfif isDefined('arguments.numberoftimesfornameid')>
			,NUMBEROFTIMESFORNAMEID=<cfqueryparam value="#arguments.numberoftimesfornameid#" cfsqltype="cf_sql_integer">
			</cfif>
			<cfif isDefined('arguments.allowenddate')>
			,ALLOWENDDATE=<cfqueryparam value="#arguments.allowenddate#" cfsqltype="cf_sql_varchar">
			</cfif>
			<cfif isDefined('arguments.active')>
			,ACTIVE=<cfqueryparam value="#active#" cfsqltype="cf_sql_varchar">
			</cfif>
			WHERE SURVEYID=<cfqueryparam value="#arguments.surveyid#" cfsqltype="cf_sql_varchar">
		</cfquery>
	</cffunction>
	...
	<cffunction name="addSurvey" access="public" returntype="numeric" hint="I add a new survey to the database">
		...
		<cfargument name="allowenddate" required="false" type="string" default="0" hint="1 if end date should be used, 0 otherwise">
		<cfargument name="active" required="false" type="string" default="0" hint="1 if the survey should be activated, 0 otherwise ">
		<cfargument name="numberoftimesfor1ip" required="false" type="string" hint="no of times a computer is allowed to take part in survey without loggin in">
		<cfargument name="numberoftimesfornameid" required="false" type="string" hint="no of times a registered user is allowed to take part in survey">
	...
	<cffunction name="addQuestion" access="public" returntype="string" hint="I add a new question for a survey to the database.">
		...
		<cfquery name="addsurvey" datasource="#arguments.surveydsn#">
	...
	<cffunction name="getAllPeopleSurveyed" access="public" returntype="query" hint="I return all people surveyed">
		<cfargument name="surveydsn" required="true" type="string" hint="database name">
		<cfargument name="surveyid" required="true" type="string" hint="id of the survey">
		<cfset var get=0>
		<cfquery name="get" datasource="#arguments.surveydsn#">
			SELECT N.FIRSTNAME, N.LASTNAME, P.PEOPLESURVEYEDID, P.NAMEID FROM PEOPLESURVEYED P
			LEFT JOIN NAME N
			ON N.NAMEID=P.NAMEID
			WHERE P.SURVEYID=<cfqueryparam value="#arguments.surveyid#" cfsqltype="cf_sql_varchar">
		</cfquery>
		<cfreturn get>
	</cffunction>
	...
	<cffunction name="getSurveyQuestions" access="public" returntype="query" hint="Get survey questions using surveyid. Return fields: QUESTIONID, SURVEYQUESTION">
		...
		<cfquery name="getsurveyquestions" datasource="#arguments.surveydsn#">
			SELECT 
				QUESTIONID, 
				SURVEYQUESTION, 
				MULTIPLEANSWERS, 
				...
				DISPLAY, 
				SORTORDER,
				(SELECT COUNT(ANSWERCHOICEID) FROM ANSWERCHOICES WHERE QUESTIONID=Q.QUESTIONID) AS NOOFANSWERCHOICES
			FROM QUESTIONS Q
			WHERE SURVEYID=<cfqueryparam value="#arguments.surveyid#" cfsqltype="cf_sql_varchar">
	...
	<cffunction name="getAnswerChoices" access="public" returntype="query" hint="Get answer choices for a particular survey question. Return fields:ANSWERCHOICEID, ANSWER">
		...
		<cfquery name="getanswerchoices" datasource="#arguments.surveydsn#">
			SELECT 
				ANSWERCHOICEID, 
				ANSWER,
				(SELECT COUNT (*) FROM RESULTS WHERE QUESTIONID=<cfqueryparam value="#arguments.questionid#"> AND ANSWERCHOICEID=A.ANSWERCHOICEID) AS RESULTCOUNT
			FROM ANSWERCHOICES A
			WHERE QUESTIONID=<cfqueryparam  value="#questionid#" cfsqltype="cf_sql_varchar">
		</cfquery>
	...
	<cffunction name="getAllTextAnswers" access="public" returntype="query" hint="I get all answerchoices and result for the survey">
		<cfargument name="surveydsn" required="true" type="string" hint="database name">
		<cfargument name="questionid" required="true" type="string" hint="id of the survey">
		<cfargument name="noofresults" required="false" type="string" default="0" hint="no of results you want">
		<cfset var get=0>
		<cfquery name="get" datasource="#arguments.surveydsn#">
			SELECT <cfif arguments.noofresults GT 0>TOP 2</cfif> 
			ANSWER FROM ANSWERS
			WHERE QUESTIONID=<cfqueryparam value="#arguments.questionid#"> 
			AND DATALENGTH(ANSWER)>0
			AND ANSWER IS NOT NULL
		</cfquery>
		<cfreturn get>
	</cffunction>
	
	<cffunction name="getTextAnswerCount" access="public" returntype="string" hint="I get the count of text answer">
		<cfargument name="surveydsn" required="true" type="string" hint="Datasource">
		<cfargument name="questionid" required="true" type="string" hint="questionid of the survey">
		<cfset var get=0>
		<cfquery name="get" datasource="#arguments.surveydsn#">
	...
	<cffunction name="isSurveyEmpty" access="public" returntype="numeric" hint="I check if people has been surveyed at all">
		<cfargument name="surveydsn" required="true" type="string" hint="Datasource">
		<cfargument name="surveyid" required="true" type="string" hint="surveyid of the survey">
		
		<cfinvoke component="survey" method="getSurveyQuestions" returnvariable="surveyquestions" surveydsn="#arguments.surveydsn#" surveyid="#arguments.surveyid#">
		
		<cfif surveyquestions.recordcount gt 0>
			<cfloop query="surveyquestions">
				<cfquery name="getresult" datasource="#arguments.surveydsn#">
	...
	<cffunction name="getrelatedquestioninfo" access="public" returntype="query" hint="I get information about related question">
		<cfargument name="surveydsn" required="true" type="string" hint="database name">
		<cfargument name="qrelationid" required="true" type="string" hint="id of the related questions">
		<cfset var get = 0>
		<cfquery name="get" datasource="#arguments.surveydsn#">
			SELECT 
				QRELATION.QRELATIONID,
				...
				QUESTIONS.SURVEYQUESTION,
				QUESTIONS.SURVEYID
			FROM QRELATION, QUESTIONS
			WHERE QRELATION.QRELATIONID=<cfqueryparam value="#arguments.qrelationid#">
			AND QRELATION.QUESTIONID=QUESTIONS.QUESTIONID
		</cfquery>
		<cfreturn get>
	</cffunction>
	...
	<cffunction name="deleterelation" access="public" returntype="string" hint="I delete related question relationship">
		...
		<cfquery name="delete" datasource="#arguments.surveydsn#">
			DELETE FROM QRELATION WHERE QRELATIONID IN (<cfqueryparam value="#qrelationidList#">)
		</cfquery>
		<cfreturn 1>
	</cffunction>

	<cffunction name="getQuestionAndSurveyName" access="public" returntype="query" hint="I return questionid, question, surveyid, survey">
		<cfargument name="surveydsn" required="true" type="string" hint="Datasource">
		<cfargument name="questionid" required="true" type="string" hint="questionid of the survey">
		<cfset var get = 0>
		<cfquery name="get" datasource="#arguments.surveydsn#">
			SELECT Q.QUESTIONID, Q.SURVEYQUESTION, S.SURVEYID, S.SURVEYNAME
			FROM QUESTIONS Q, SURVEY S
			WHERE Q.QUESTIONID=<cfqueryparam value="#arguments.questionid#"> 
			AND Q.SURVEYID=S.SURVEYID
		</cfquery>
		<cfreturn get>
	</cffunction>
	
	<cffunction name="getAnswerChoiceInfo" access="public" returntype="query" hint="I return answerchoiceid, answerchoice, questionid, surveyquestion, surveyid, surveyname">
		<cfargument name="surveydsn" required="true" type="string" hint="Datasource">
		<cfargument name="answerchoiceid" required="true" type="string" hint="id of the answerchoice">
		<cfset var get = 0>
		<cfquery name="get" datasource="#arguments.surveydsn#">
			SELECT 
				...
				S.SURVEYNAME,
				S.SURVEYID
			FROM ANSWERCHOICES A,QUESTIONS Q, SURVEY S
			WHERE ANSWERCHOICEID=<cfqueryparam value="#arguments.answerchoiceid#">
	...
	<cffunction name="isThisAValidPeopleSurveyedId" access="public" returntype="boolean" hint="id of the people surveyed">
		<cfargument name="surveydsn" required="true" type="string" hint="database name">
		<cfargument name="surveyid" required="true" type="String" hint="id of the survey">
		<cfargument name="peoplesurveyedid" required="true" type="string" hint="id of the people surveyed">
		<cfset var get=0>
		<cfquery name="get" datasource="#arguments.surveydsn#">
			SELECT COUNT(*) AS N FROM PEOPLESURVEYED
			WHERE SURVEYID=<cfqueryparam value="#arguments.surveyid#" cfsqltype="cf_sql_varchar">
			AND PEOPLESURVEYEDID=<cfqueryparam value="#arguments.peoplesurveyedid#" cfsqltype="cf_sql_varchar">
		</cfquery>
		<cfif get.N GT 0>
			<cfreturn TRUE>
		<cfelse>
	...
	<cffunction name="getmytextanswer" access="public" returntype="String" hint="I get text answer for a question">
		<cfargument name="surveydsn" required="true" type="string" hint="Datasource">
		<cfargument name="questionid" required="true" type="string" hint="questionid of the survey">
		<cfargument name="peoplesurveyedid" required="true" type="string" hint="nameid of the person">
		<cfset var myanswer=0>
		<cfquery name="myanswer" datasource="#arguments.surveydsn#">
			SELECT ANSWER FROM ANSWERS
			WHERE QUESTIONID=<cfqueryparam value="#questionid#" cfsqltype="cf_sql_varchar">
			AND PEOPLESURVEYEDID=<cfqueryparam value="#arguments.peoplesurveyedid#" cfsqltype="cf_sql_varchar">
		</cfquery>
	...
	<cffunction name="getMyResult" access="public" returntype="Query" hint="I get answers of a question for a person">
		<cfargument name="surveydsn" required="true" type="string" hint="Datasource">
		<cfargument name="questionid" required="true" type="string" hint="questionid of the survey">
		<cfargument name="peoplesurveyedid" required="true" type="string" hint="questionid of the survey">
		<cfset var result=0>
		<cfquery name="result" datasource="#arguments.surveydsn#">
			SELECT RESULTS.ANSWERCHOICEID, ANSWERCHOICES.ANSWER
			FROM RESULTS, ANSWERCHOICES
			WHERE RESULTS.QUESTIONID=<cfqueryparam value="#arguments.questionid#" cfsqltype="cf_sql_varchar">
			AND RESULTS.ANSWERCHOICEID=ANSWERCHOICES.ANSWERCHOICEID
			AND RESULTS.PEOPLESURVEYEDID = <cfqueryparam value="#arguments.peoplesurveyedid#"> 
		</cfquery>
		<cfreturn result>
	</cffunction>  
	
	<cffunction name="getPersonInfo" access="public" returntype="query" hint="I get information about the person surveyed">
		<cfargument name="surveydsn" required="true" type="string" hint="database name">
		<cfargument name="surveyid" required="true" type="String" hint="id of the survey">
		<cfargument name="peoplesurveyedid" required="true" type="string" hint="id of the people surveyed">
		<cfset var get=0>
		<cfquery name="get" datasource="#arguments.surveydsn#">
			SELECT FIRSTNAME, LASTNAME FROM NAME N, PEOPLESURVEYED P
			WHERE N.NAMEID=P.NAMEID
			AND P.PEOPLESURVEYEDID=<cfqueryparam value="#arguments.peoplesurveyedid#" cfsqltype="cf_sql_varchar">
		</cfquery>
		<cfreturn get>
	</cffunction>

	<cffunction name="deleteSurvey" access="public" returntype="boolean" hint="Delete Survey">
		<cfargument name="surveydsn" required="true" type="string" hint="Datasource">
		<cfargument name="surveyid" required="true" type="string" hint="surveyid of the survey">
		<cfset var delete=0>
		<cfset var get=0>
		<cfset var existancetest=0>
		<cfquery name="existancetest" datasource="#arguments.surveydsn#">
			SELECT COUNT(*) AS SURVEYEXISTS FROM SURVEY
			WHERE SURVEYID=<cfqueryparam value="#arguments.surveyid#">
		</cfquery>
		<cfif existancetest.SURVEYEXISTS GT 0>
			<cfquery name="get" datasource="#arguments.surveydsn#">
			...
			<cfif get.N EQ 0>
				<cfquery name="delete" datasource="#arguments.surveydsn#">
					DELETE FROM SURVEY WHERE SURVEYID=<cfqueryparam value="#arguments.surveyid#">
				</cfquery>
				<cfreturn 1>
			<cfelse>
				<cfreturn 0>
