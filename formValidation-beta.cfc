					...
					<cfelseif #REFind("^min_val\[(.*?)]",i)#>
						<cfif requiredfield EQ 1 OR IsNumeric(form['#arguments.field#'])>
							...
							<cfif form['#arguments.field#'] LT min_value>
								<cfset variable.validTest=false>
					...
					<cfelseif #REFind("not\[(.*?)]",i)#>
						<cfif requiredfield EQ 1>
							...
							<cfif form['#arguments.field#'] EQ notValue>
								<cfset variable.validTest=false>
								<cfset variables.validation_error['#arguments.field#']="#arguments.humanField# can not be #notValue#. If this has been your password please contact administrator to change this password." >
							</cfif>					
						</cfif>
					</cfif>
